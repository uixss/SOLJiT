...
try:
    ...
    # Permite correr el módulo solo (sin core/) si así lo deseas
...
def get_quote(input_mint: str, output_mint: str, amount: int, slippage_bps: int) -> Optional[Dict[str, Any]]:
    try:
        params = {
            ...
            "onlyDirectRoutes": "false" if ALLOW_MULTI_HOPS else "true",
        }
...
def swap(input_mint: str, output_mint: str, amount_lamports: int, slippage_bps: int) -> bool:
    ...
    try:
        ...
        if USE_JITO and send_v0_with_jito_bundle is not None:
            res = send_v0_with_jito_bundle(signed_tx, payer)
